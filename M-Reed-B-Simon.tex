...
matematycznej'', str.~102 i~dalsze, \cite{Sch79}. W~szczególności tak
jak dla odwzorowania liniowego odwzorowanie wieloliniowe jest ciągłe
\wtw, gdy jest ograniczone, analogicznie również definiuje~się normę
...
całki w~\cite{Schwartz79}, będę zawsze przyjmował, że~mierzalna
funkcja dodatnia jest zawsze całkowalna, choć jej całka możne wynosić
$+\infty$. Jego podejście wskazuje, że~dla funkcji dodatniej
mierzalność nie jest potrzebna, wprowadza więc on~dodatkowe pojęcia
całki górnej, która istniej dla każdej funkcji $f \geq
0$. % Sprawdzone.
...
Istotą samego pojęcia osobliwości miar jest fakt, że~miara $\nu$ jest
niezerowana\footnote{Przyjęło~się mówić, że~miara jest \tb{skupiona},
  ale na razie nie widziałem, by~Reed i~Simon tego pojęcia używali.
  Zob.~str.~133, \cite{Rudin98}} tylko na zbiorze o~$\mu$\dywiz
mierze~0. Formalnie rzecz biorąc, istnieje taki zbiór~$A$,
że~$\mu( A ) = 0$, jednocześnie $\nu( M \setm A ) = 0$ ($M$ oznacza tu
przestrzeń mierzalną). Oznacza to, że~$\nu$ może być różna od~zera
...
symetryczna. Obie definicje~są więc równoważne.
...
pomimo, że~to udowodnił. Tym samym, całka ta zawsze ma sens i~jak
zauważył Rudin, dzięki $\si$\dywiz skończoności można pokazać, że~jest
równa $\nu( A )$.
...
$d_{ i } \in \C$, korzystając z~półtoraliniowości mamy
\begin{equation*}
  \begin{split}
  ...
  \end{split}
\end{equation*}
$\mu( \eta )$ oznacza wartość formy $\mu$ na~$\eta$. Sumę elementów
$\Hc_{ i } \ti \Hc_{ 2 }$ definiujemy nadając temu iloczynowi
kartezjańskiemu strukturę sumy prostej
\begin{equation}
  \label{eq:4}
  \eta = \Sum_{ i = 1 }^{ N } d_{ i } \, \lket \phi_{ i }, \, \vp_{ i } \rket
...
\end{equation*}
Jak łatwo jednak sprawdzić, iż formy
...
względem odpowiedniej relacji równoważności\footnote{Więcej o~tym
  niżej.}, niż~po~prostu zbiorem sum odpowiednich form prostych,
z~jakiegoś jednak względu Reed i~Simon uznali, że~nie warto
zagłębiać~się w~tą kwestię.
...
Kluczowe tu jest fakt, że~dwie różne postaci tej samej formy różnią
się o~sumę form prostych reprezentującą formę zerową. Jeżeli weźmiemy
dwie formy określone wyżej $\mu_{ 1 }$ i~$\mu_{ 2 }$, to ich różnica:
\begin{equation*}
...
w~$\mc{E}$, to~niezależność iloczynu skalarnego $\mc{E}$ od~konkretnej
postaci form będzie wykazana.
...
Ta relacja stanowi po~prostu, że~dwa elementy $\mu_{ 1 }, \mu_{ 2 } \in \mc{E}$ są równoważne, wtedy i~tylko wtedy
