...
niekoniecznie borelowskich, określonym na wspólnej% wspólnym..... pierścieniu <<<<
 $\s$\dywiz
pierścieniu $\Rc$, określamy \tb{sumę miar} jako:
\begin{equation}
  \label{eq:RSI1}
  \begin{split}
...
\begin{proof}
  Suma miar jest określona na~wspólnych %wspólnym <<<<<<<<<<<
   ich pierścieniu $\Rc$, poza
  zaś warunkiem na~przeliczalną addytywność miary rozłącznych zbiorów
  $A_{ i }$, pozostałe własności miary dodatniej wychodzą od razu.

  Załóżmy teraz, że~miary wszystkich tych zbiorów względem $\mu_{ 1 }$
  i~$\mu_{ 2 }$ są skończone. Na podstawie podstawowych twierdzeń
  o~sumowaniu szeregów dodatnich, mamy \Dok
  \begin{equation*}
    \begin{split}
    ...
    \end{split}
  \end{equation*}
  Jeśli chodź dla jednego zbioru mamy
  $\mu_{ n }( A_{ i } ) = +\infty$, to obie strony są równe $+\infty$.
  Problem, czy można dodać dwie miary, które nie są dodatnie wymaga
  osobnych rozważań. \Dok

  Rozpatrzmy teraz problem całkowania względem sumy miar $\mu_{ n }$
  ...
  \begin{equation}
    ...
    \end{split}
  \end{equation}
  Znowu wszystkie wyrazy szeregów są dodatnie, więc powyższe
  przekształcenia są uzasadnione.
...
duże wartości jest mała względem $\mu_{ 1 }$. Jeżeli miara tego zbioru
względem $\mu_{ 2 }$ jest jednak duża, funkcja $f_{ 1 }$ może być
niecałkowalna względem tej miary.
...
\begin{equation*}
  ...
  \end{cases}
\end{equation*}
$f \equiv 0$, jest ona całkowalna względem miary Lebesgue'a $dx$ i~jej
całka wynosi 0. Możemy zmienić jej wartość w~0 na $-\infty$, bez
żadnych konsekwencji dla wartości całki po~$dx$. Jeżeli teraz dodamy
do miary Lebesgue'a miarę Diraca skupioną w~0, oznaczmy ją
$d\del_{ 0 }$, to $f$ jest niecałkowalna względem miary
$dx + d\del_{ 0 }$.
...
W~tym rozdziale natykamy kilkakrotnie problem, że~mamy funkcje ciągłą
$f( x )$ oraz~ciąg $x_{ n_{ 1 }, \ld, n_{ m } }$ i~wchodzimy wejść % nie rozumiem co ma tu znaczyć zwrot >>wchodzimy wejść <<<, sensowne wydaje się tu  >>>> próbujemy wejść <<<<
z~granicą ciągu pod funkcję, innymi słowy chcemy, aby
zachodziło\footnote{Rozpatrujemy tu tylko przypadek skończonej liczby
  ...
  rozpatrywać ciągi% czy nie powinno być >>>ciągów<<<?
   o~nieskończonej ilości indeksów lub~takich które
  nie są indeksowane liczbami naturalnymi.}
\begin{equation*}
  \Lim_{ \sstack{ n_{ 1 } \ra +\infty \\ \cdots \\  n_{ m } \ra +\infty } }
  f( x_{ n_{ 1 }, \ld, n_{ m } } )
  = f( \Lim_{ \sstack{ n_{ 1 } \ra +\infty \\ \cdots \\ n_{ m } \ra +\infty } }
  x_{ n_{ 1 }, \ld, n_{ m } } ).
\end{equation*}
...
\end{equation*}
to czy twierdzenie to również zachodzi? W~przypadku ogólnym nie wiem,
czy tak jest, bowiem gdy przestrzeń topologiczna nie~ma przeliczalnej
bazy otoczeń, to~ciągi postaci $x_{ n }$ nie zawierają pełnej wiedzy
o~topologii. Udowodnimy jednak, że~tak jest dla przestrzeni
metrycznych.

Na początku będzie nam potrzebna definicja granicy ciągu
wieloindeksowego. Najprostsza definicja granicy jest
następująca\footnote{Granica ciągu wieloindeksowego powinna być
  szczególnym przypadkiem granic indeksowanych zbiorami skierowany % sierowanymi <<<<?
  które~są omawiane u~Marina \cite{Mau74}.}. Niech $X$ będzie
przestrzenią metryczną, $x_{ n_{ 1 }, \ld, n_{ m } }, x_{ 0 } \in X$,
$x_{ 0 }$ jest granicą ciągu $x_{ n_{ 1 }, \ld, n_{ m } }$ \wtw, gdy
\begin{displaymath}
...
\begin{twr}
  ...
  \begin{equation}
    \Lim_{ \sstack{ n_{ 1 } \ra +\infty \\ \cdots \\  n_{ m } \ra +\infty } }
    f( x_{ n_{ 1 }, \ld, n_{ m } } )
    = f( \Lim_{ \sstack{ n_{ 1 } \ra +\infty \\ \cdots \\ n_{ m } \ra
...
rozdziela % chyba powino być >>>W tym rozdziale <<<
 potrzebujemy kilka razy pokazać równość następującego typu.
...
\begin{twr}
  ...
  \begin{equation*}
    ...
    \Sum_{ \sstack{ l_{ 1 } < N \\ \cdots \\ l_{ n } < N } }x
...
\begin{proof}
  ...
  $\sum_{ k < K } c_{ k } \vp_{ k }$ jest zbieżna %czy >>zbieżny<< - chodzi tu o ciąg czy sumę?
   do~$\vp$, wynika,
  że~ ciąg
  $\norm{ \sum_{ k < K } c_{ k } \vp_{ k } }_{ X_{ 1 } } \ra \norm{
    \vp }_{ X_{ 1 } }$, jest więc ograniczony. Dobierz% Dobierzmy -<<<by było w zgodnośc z resztą....
     $M$ tak~by było
  wspólnym ograniczeniem ciągów
...
,,thus'', że~podane dwie definicje normy operatora ograniczonego~są %propozycja >>>że obie podane definicie<<<<
...
jest to~prawdą, więc ze~względy % ze~względu<<<<
 na~fakt,
iż~$V_{ 1 } \subset \wt{ V }_{ 1 }$, mamy nierówność
...
przeprowadzona zbyt szybko, by~dawała satysfakcję. Najpierw należałoby% nie wiem co ma znaczyc konstrukcja  >>należałoby by<< niewystarczy samo >> należałoby<<
...
oczywiste. Istotnie, jak zauważyli Reed i~Simon, przy definicje %definicji <<<
 zwykle
spotykanej w~pracach dla matematyków, tego typu dowody~są prostsze.
Wiedząc już jednak o~tym, widać od~razu, że~jeżeli
$\Int{ \abs{ f } }{ x } < \infty$ to całkowalne~są również $f_{ + }$
...
Nikodyma obie miary~są %powtórzenie >>obie  miary <<< zdecyduj które lepiej wyrzucic , przed czy po?
 dodatnie, można więc zagwarantować,
że~$f \geq 0$,% ja w tym miejscu zaczął bym nowe zdanie
 całka z~tezy ma więc zawsze sens i~wynik może być
nieskończony. W~tym kontekście warto mieć w~pamięci rozróżnienie jakie
podał Schwartz między całką górną, która jest zdefiniowana dla
wszystkich funkcji nieujemnych i~zawsze ma sens, a~całką w~sensie
właściwym która wymaga by~$\Int{ | f( x ) | }{ x }$ była skończona
...
ze~zmiennych z~osobna. Dla miar Radona, dla~których który zbiory %powtórzenie który ???? <<<<<<<<<<<<
borelowskie~są mierzalne, tak zapewne jest, jednak dla ogólnych
$\s$\dywiz algebr wydaje mi~się dziwnie mocny twierdzeniem %propozycja>>wydaje mi się to dziwnie mocnym twierdzeniem <<<
. Zapewne
zachodzi twierdzenie, że~funkcja jest prawie wszędzie mierzalna. \Dok
...
algebry: odwzorowanie liniowe jest injektywne \wtw gdy~jego jądro jest % tu chyba brakuje odstępu pomiedzy \wtw i gdy, nie wiem czy nie będziesz musiał zmienić komendy \wtw by to naprawić <<<<<<<<<<
...
ortogonalny niezerowych wektorów $S$ jest zupełny \wtw gdy jedynym %jw <<<<<
...
w~kierunku prostopadły, %prostopadłym<<<<
 do~wszystkich innych, żadne da %dwa?<<<<<<<
  nie mogą~się
więc kompensować. Szereg ten jest więc zbieżny niezależnie
od~kolejności bądź rozbieżny.
...
W~dalszym ciągu potrzebny %potrzebna<<<<<<<<<<
 będzie nam następująca obserwacja: jeżeli
...
Teraz możemy przejść do~dyskusje, %do dyskusji<<<<<<<
 czy~wzór na iloczyn skalarny nie
...
przestrzeń są metryczne, to~odwzorowanie jest topologiczne ciągła % ciągłe?<<<<<<
 \wtw%jw<<<<
...
ona ciąg Cauchy'ego i~tym samym jest ograniczony. Dobierz % znów proponuje zmianę na >>Dobierzmy<<<
 $M$ tak~by
było wspólnym ograniczeniem ciągów $\sum_{ l < L } \vp_{ l }$
i~$\sum_{ k < K } \psi_{ k }$. Tym samym
\begin{displaymath}
  \begin{split}
