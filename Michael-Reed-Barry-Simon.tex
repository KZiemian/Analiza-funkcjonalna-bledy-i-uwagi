...
\usepackage{latexshortcuts} \usepackage{mathshortcuts}
...
niekoniecznie borelowskich, określonym na wspólnym $\sigma$\dywiz
...
\begin{proof}
  ...
  \begin{equation}
    \label{eq:RSI3}
    \IntMo{ f }{ ( \mu_{ 1 } + \mu_{ 2 } ) } = \IntMo{ f }{ \mu_{ 1 } }
    + \IntMo{ f }{ \mu_{ 2 } }.
  \end{equation}
...
\begin{equation*}
  \Lim_{ \substack{ n_{ 1 } \to +\infty \\ \cdots \\  n_{ m } \to +\infty } }
  f( x_{ n_{ 1 }, \ld, \, n_{ m } } )
...
\begin{twr}
  ...
  granica ciągu
  $\Lim_{ \substack{ n_{ 1 } \to +\infty \\ \cdots \\ n_{ m } \to
      +\infty } } x_{ n_{ 1 }, \ld, \, n_{ m } } = x_{ 0 }$, to
  \begin{equation}
    \Lim_{ \substack{ n_{ 1 } \to +\infty \\ \cdots \\  n_{ m } \to +\infty } }
    f( x_{ n_{ 1 }, \ld, \, n_{ m } } )
    = f( \Lim_{ \substack{ n_{ 1 } \to +\infty \\ \cdots \\ n_{ m } \to
...
$c_{ l }, d_{ k } \in \Cc$, to
\begin{equation*}
  \Lim_{ K,\, M \to +\infty } \Sum_{ \substack{ l < L \\ k < K } }
...
\cite{SchwartzKursAnalizyMatematycznej1979}. W~szczególności tak jak dla
...
\begin{twr}
  ...
  Jeśli $B: X_{ 1 } \ti \ld \ti X_{ n } \to Y$ jest odwzorowanie
...
\begin{proof}
  ...
  że~ciąg
  $\norm{ \sum_{ k < K } c_{ k } \vp_{ k } }_{ X_{ 1 } } \to \norm{
...
wyrazów\footnote{Co jest prawdą pod warunkiem, że~iloczyn szeregów
  ...
  \cite{SchwartzKursAnalizyMatematycznej1979}.}.
...
jakiego używa Fichtenholza (zobacz str.~??? \cite{FichtenholzRachunekRiCTomII2004})
...
$0 \leq g \leq f$, to z~tego, że~$\IntMo{ f }{ x } < \infty$, wynika
że~$\IntMo{ g }{ x } < \infty$, co przy przyjętej tu definicji całki
...
widać od~razu, że~jeżeli $\IntMo{ \abso{ f } }{ x } < \infty$ to
...
całki w~\cite{SchwartzKursAnalizyMatematycznej1979}, będę zawsze
...
i~jak zauważył Rudin, dzięki $\sigma$\dywiz skończoności można
...
$\sigma$\dywiz algebr wydaje mi~się to dziwnie mocny twierdzeniem.
...
$\abso{ f_{ n }( x ) - f_{ n }( y ) } < \veps$ zachodzi dla
każdego~$n$, to również $\abso{ f( x ) - f( y ) }$. Będzie nam to
...
\begin{proof}
  ...
  \item Jeśli zbiór $\Dc$ jest gęsty, więc dla każdego $x \in \Hc$
    można znaleźć ciąg $x_{ n } \in \Dc$ taki, że~$x_{ n } \to x$. Tym
...
$\sum_{ i = 1 }^{ N } \abso{ \SP{ x_{ \al_{ i } } }{ y } } \leq \norm{
...
\cite{SchwartzKursAnalizyMatematycznej1979}). Trajdos przetłumaczył ich
nazwę jako szeregi \emph{zbieżne przestawialnie}. % Sprawdzone.
...
\begin{equation*}
  \begin{split}
    \SP{ y }{ y' } &= \SP{ y }{ \Lim_{ n \to \infty } \Sum_{ j = i }^{
        n } \SP{ x_{ \al_{ j } } }{ y' } x_{ \al_{ j } } } = \Lim_{ n
      \to \infty } \SP{ y }{ \Sum_{ j = 1 }^{ n } \SP{ x_{ \al_{ j } }
      }{ y' } x_{ \al_{ j } } } \\
    &= \Lim_{ n \to \infty } \Sum_{ j = 1 }^{ n } \SP{ y }{ \SP{ x_{
...
\begin{equation*}
  ...
  \SP{ x_{ \al } }{ y } = \Sum_{ \al \in A } \abso{ \SP{ x_{ \al } }{ y } }^{ 2 }.
\end{equation*}
% Sprawdzone.
...
\begin{twr}[Ogólna postać nierówności Bessela]
  ...
  \begin{equation*}
    \Sum_{ \be \in B } \abso{ \SP{ x_{ \be }}{ x } }^{ 2 } \leq \norm{ x }^{ 2 }.
  \end{equation*}
\end{twr}
\begin{proof}
  Prawie nie ma czego dowodzić, bowiem
  \begin{equation*}
    \norm{ x }^{ 2 } = \Sum_{ \al \in A } \abso{ \SP{ x_{ \al } }{ x } }^{ 2 }
    \geq \Sum_{ \be \in B } \abso{ \SP{ x_{ \be }}{ x } }^{ 2 }.
  \end{equation*}
\end{proof}
% Sprawdzone.
...
\begin{equation*}
  ( \la, \, \la ) = \Sum_{ j l } \abso{ c_{ j l } }^{ 2 } \geq 0,
\end{equation*}
...
\begin{twr}
  ...
  $\sigma$\dywiz skończoną i~$A \subs X \ti Y$ będzie zbiorem
  mierzalny względem miary $\mu \ot \nu$. Jeśli dla $\mu$\dywiz prawie
  każdego $x \in X$, miara $\nu$ zbioru $( x \ti Y ) \cap A$ jest
  równa 0, wówczas $(\mu \ot \nu)( A ) = 0$.
\end{twr}
\begin{proof}
  ...
  miary~są $\sigma$\dywiz skończone, to zachodzi twierdzenie
...
\begin{subequations}
  \begin{equation}
    \label{eq:7}
    \iint\limits_{ M_{ 1 } \ti M_{ 2 } } \abso{ \vpk( x ) \psil( y ) }^{ 2 } \,
    d\mu_{ 1 }( x ) d\mu_{ 2 }( y ) = \Int_{ M_{ 1 } } \abso{ \vpk( x ) }^{ 2 }\,
    d\mu_{ 1 }( x ) \Int_{ M_{ 2 } } \abso{ \psil( y ) }^{ 2 }\, d\mu_{ 2 }( y )
    = 1.
  \end{equation}
...
w~przestrzeni $\Lt( M_{ 1 } \ti M_{ 2 }, d\mu_{ 1 } \ot d\mu_{ 2 } )$,
...
$f^{ -1 }( \Cc \setm 0 )$, jest więc on mierzalny. Teraz fakt,
...
\begin{center}
  \begin{tabular}{|c|c|c|c|c|}
    ...
    50  & 16 & & $\Sum_{ j l } \abso{ c_{ j l } }^{ 2 }$
           & $\Sum_{ j l } \abso{ c_{ j l } }^{ 2 } \geq 0$ \\
    53  & & 12 & $\vp_{ k_{ \sigma( 1 ) } } \ot \vp_{ k_{ \sigma( 2 ) } } \cdots
                 \ot \vp_{ k_{ n( p ) } }$
           & $\vp_{ k_{ \sigma( 1 ) } } \ot \vp_{ k_{ \sigma( 2 ) } } \ot \cdots
             \ot \vp_{ k_{ \sigma( p ) } }$ \\
    70  & 8 & & $\Lim_{ n \to +\infty }$ & $\Lim_{ m \to +\infty }$ \\ % Sprawdź czy
    % dobrze.
    73  & & 2 & $\Sum_{ k = 1 }^{ \infty } \abso{ \la_{ k } } < \infty$
           & $\Sum_{ k = 1 }^{ \infty } \abso{ \la_{ k } } \leq
