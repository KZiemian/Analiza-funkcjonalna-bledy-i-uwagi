...
\begin{proof}
  ...
  \begin{equation}
    \label{eq:RSI3}
    \IntM{ f }{ ( \mu_{ 1 } + \mu_{ 2 } ) } = \IntM{ f }{ \mu_{ 1 } }
    + \IntM{ f }{ \mu_{ 2 } }.
  \end{equation}
...
$0 \leq g \leq f$, to z~tego, że~$\IntM{ f }{ x } < \infty$, wynika
że~$\IntM{ g }{ x } < \infty$, co przy przyjętej tu definicji całki
...
widać od~razu, że~jeżeli $\IntM{ \abso{ f } }{ x } < \infty$ to
...
\begin{equation*}
  \IntM{ | f( p ) - f_{ n }( p ) | }{ p } = \IntM{ ( f( p )
    - f_{ n }( p ) ) }{ p } = \IntM{ f( p ) }{ p }
  - \IntM{ f_{ n }( p ) }{ p }
\end{equation*}
Dowód, że~przy podanych w~tym twierdzeniu założeniach zachodzi
$f \in \LIo$ i~$\IntM{ f_{ n } }{ p } \to \IntM{ f }{ p }$ można
...
i~jak zauważył Rudin, dzięki $\sigma$\dywiz skończoności można pokazać,
że~jest równa $\nu( A )$. % Sprawdzone.
...
\cite{SchwartzKursAnalizyMatematycznej79}). Trajdos przetłumaczył ich
...
\begin{twr}
  ...
  $\sigma$\dywiz skończoną i~$A \subs X \ti Y$ będzie zbiorem mierzalny
...
\begin{proof}
  ...
  miary~są $\sigma$\dywiz skończone, to zachodzi twierdzenie o~zamianie
