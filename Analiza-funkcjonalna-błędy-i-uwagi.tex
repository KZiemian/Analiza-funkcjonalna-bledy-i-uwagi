...
również \textit{zbiór zerowy dystrybucji}.
...
\start \Str{48--49} \textbf{Lemat 1, uzupełnienie.} Na~podstawie
...
\begin{equation}
  \label{eq:Zem-s01-06}
  M =
  \left( \sum_{ i = 0 }^{ n } m_{ i } \right)^{ \frac{ 1 }{ 2 } }.
\end{equation}
...
potrzebujemy udowodnić zbieżność jednostajną tego ilorazy do~$-\varphi^{ ( 1 ) }( t )$
oraz jednostajną zbieżność jego $k$-pochodnej
...
% %          & $-( \lambda_{ 0 } - \lambda )^{ n } f( \plambda_{ 0 } )^{ n + 1 }$ \\
% %   & & & & \\
% %   \hline
% % \end{tabular}
